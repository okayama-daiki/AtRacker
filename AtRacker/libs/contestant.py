...
Result = namedtuple("Result", ("name", "OldRating", "NewRating", "Performance"))
...
class Contestant(object):
    ...
    def identify_latest_abc_holding_number() -> None:
        ...
        return re.findall(
            r"AtCoder Beginner Contest [0-9]{3}", bs.find_all("td")[1].find("a").text
        )[0][-3:]
    ...
    def rank_contestants_by_achive(cls, abc_holding_number: str = None) -> list[tuple]:
        ...
        for name, obj in cls.member.items():
            try:
                result = obj.extract_specific_contest_result(abc_holding_number)
    ...
    def rank_contestants_by_highest_rating(cls) -> list[tuple]:
        return sorted(
            ((name, obj.highest_rating) for name, obj in cls.member.items()),
            key=lambda x: -x[1],
        )
    ...
    def rank_contestants_by_current_rating(cls) -> list[tuple]:
        return sorted(
            ((name, obj.current_rating) for name, obj in cls.member.items()),
            key=lambda x: -x[1],
        )
    ...
    def detect_color_change(results: list[Result]) -> list[str]:
        ...
        for result in results:
            if bisect(COLOR_BORDER, result.OldRating) < (
                new := bisect(COLOR_BORDER, result.NewRating)
            ):
