...
USER_WITH_CONTEST_RESULT_HYEPER_LINK = 'atcoder.jp/contests/abc{no}/standings?watching={name}:{name}>'
...
def visualize_ranking(ranking: list[tuple], order_limit=3) -> str:
    if ranking:
        return '\n'.join(fmt.format(*rank) for rank, fmt
                         in zip(ranking[:order_limit], RANKING_FORMAT))
    else:
...
def display_archiver(name: str, contest_no: int | str) -> str:
    return f'<https://atcoder.jp/contests/{contest_no}/standings?watching={name}|{name}>'
...
def display_result(results) -> str:
    if results:
        return CONTEST_RESULT_FORMAT\
            + ''.join(CONTEST_USER_RESULT_FORMAT.format(whole_space(name, 15),
                                                        whole_space(f'{old} -> {new} ({new-old:+})', 21), perf)
